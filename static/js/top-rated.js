...
function renderTopRatedSlides() {
  ...
  if (DataManager.isLoaded) {
    items = DataManager.getPageFilteredItems('top-rated');
  } else {
    window.addEventListener('dataLoaded', renderTopRatedSlides);
    return;
  }
  ...
  const selectedSlides = getRandomTopRatedSlides(items, 8);
  ...
  if (window.setupSwiper) window.setupSwiper('#top-rated .new-content');
}
